...
def createfunct(n, c):
    # Select n random variables
    variables = sympy.symbols(f"x:{n}")

    # Generate a random function composition using SYMBOLIC_LIB from KAN
    def generate_random_function(variable, depth):
        if depth == 0:
            return variable
        else:
            func_name = random.choice(list(SYMBOLIC_LIB.keys()))
            torch_func, sympy_func, _, _ = SYMBOLIC_LIB[func_name]
            inner_func = generate_random_function(variable, depth - 1)
            return sympy_func(inner_func)

    # Create the composed function for each variable and combine them
    composed_functions = [generate_random_function(var, c) for var in variables]

    # Combine the composed functions using a random binary operator
    # we can take it or not
    def combine_functions(functions):
        if len(functions) == 1:
            return functions[0]
        combined_function = functions[0]
        for func in functions[1:]:
            operator = random.choice([sympy.Add, sympy.Mul])
            # just add or multiply
            combined_function = operator(combined_function, func)
        return combined_function

    combined_function = combine_functions(composed_functions)

    # Manually convert SymPy to a PyTorch function (dimensionwsie)
    def sympy_to_torch_func(expr, variables):
        var_symbols = [str(var) for var in variables]
        expr_str = str(expr)
        for var in var_symbols:
            expr_str = expr_str.replace(var, f"x[:, {var_symbols.index(var)}]")

        for sym_func, (torch_func, sympy_func, _, _) in SYMBOLIC_LIB.items():
            expr_str = expr_str.replace(str(sympy_func), f"torch.{torch_func.__name__}")

        lambda_str = f"lambda x: {expr_str}"
        return eval(lambda_str)

    torch_lambda_function = sympy_to_torch_func(combined_function, variables)

    # Validation, we need to check if the function gives bad values
    def validate_function(f, variables):
        test_values = torch.randn(1, len(variables))
        try:
            result = f(test_values)
            if not torch.isfinite(result).all():
                raise ValueError(f"Generated function has invalid values: {result}")
        except Exception as e:
            raise ValueError(f"Error in generated function: {e}")

    validate_function(torch_lambda_function, variables)

    return torch_lambda_function, combined_function
